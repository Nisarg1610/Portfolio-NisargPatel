...
const ResumeCard = ({ title, subTitle, result, des=[] }) => {
  return (
    <div className="w-full h-full group flex">
      <div className="w-10 h-full bgOpacity mt-16 relative">
        <span className="absolute w-5 h-5 rounded-full -top-2 -left-3 flex justify-center items-center bg-black bg-opacity-60">
          <span className="w-3 h-3 rounded-full bg-bodyColor inline-flex group-hover:bg-designColor duration-300"></span>
        </span>
      </div>
      <div className="w-full h-full bg-black bg-opacity-20 hover:bg-opacity-30 duration-300  rounded-lg p-4 lgl:px-10 flex flex-col justify-center gap-6 lgl:gap-10 shadow-shadowOne">
  ...
  {des.map((v, i) => (
    <p key={i} className="mb-2">{v}</p>  // ✅ Valid: Each <p> is inside a <div>
  ))}
</div>
